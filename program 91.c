...
void hmac_sha256(char *key, char *message,
                 unsigned char result[32])
{
    ...
    if(keyLen > 64)
    {
        sha256((unsigned char *)key,keyLen,keyBlock);
    }
    else
    {
    ...
    memcpy(inner+64,message,msgLen);

    sha256(inner,64+msgLen,innerHash);

    memcpy(inner,opad,64);
    memcpy(inner+64,innerHash,32);

    sha256(inner,96,result);
}
